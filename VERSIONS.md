...
| V0.04   | 2026-09-12 | `PENDING` | Deploy fix: added `vercel.json` with an SPA rewrite (`/(.*)` → `/index.html`). Without it Vercel resolved client-side routes against the filesystem, so `/work`, `/resume`, and every `/project/:slug` returned a 404 on direct load or refresh — only `/` worked, and internal links only worked because they never hit the server. Static assets are unaffected: Vercel matches real files before applying rewrites. |
